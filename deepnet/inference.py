...
def DoSampling(sampleNum, model_file, train_op_file, base_output_dir, layernames,
                memory='1G', method='gibbs',
                steps=1000, datasets=['validation', 'test'], gpu_mem='2G',
                main_mem='30G', data_proto=None):
  model = util.ReadModel(model_file)
  op = ReadOperation(train_op_file)
  op.randomize = False
  ...
  for dataset in datasets:
    output_dir = os.path.join(base_output_dir, dataset)
    print 'Writing to %s' % output_dir
    size = net.GetSampleFromLayers(layernames, steps, sampleNum, output_dir,
                                   all=False,  memory=memory, dataset=dataset, method=method)
