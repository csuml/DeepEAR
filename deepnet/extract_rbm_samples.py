...
def ExtractRepresentations(model_file, train_op_file, layernames,
  ...
  for dataset in datasets:
    ...
    size = net.WriteRepresentationToDisk(
      layernames, output_dir, memory=memory, dataset=dataset, input_recon=True, sampling = True, gibbs_steps = gibbs_steps)
    ...
    for i in xrange(gibbs_steps):
        for j, lname in enumerate(layernames):
          ...
          data.size = size[i*len(layernames) + j]
          data.name = '%s_%s' % (lname + "_" + str(i + 1), tag)
          data.file_pattern = os.path.join(dataset, '%s-*-of-*.npy' % (lname + "_" + str(i + 1)))
